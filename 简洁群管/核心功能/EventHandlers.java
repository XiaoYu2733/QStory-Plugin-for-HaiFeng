...
public void onMsg(Object msg) {
    if (msg.MessageContent != null && msg.MessageContent.equals("log") && 是主人(msg.UserUin)) {
    ...
    synchronized (msgLock) {
        try {
            ...
            if (msgContent.startsWith("上管") && !mAtListCopy.isEmpty()) {
                ...
                setTroopAdmin(groupUin, target, 1);
                sendMsg(groupUin, "", "已发送上管请求：" + 名(target));
                return;
            }
            if (msgContent.startsWith("下管") && !mAtListCopy.isEmpty()) {
                if (!isOwner) return;
                String target = (String) mAtListCopy.get(0);
                if (target == null || target.isEmpty()) return;
                setTroopAdmin(groupUin, target, 0);
                sendMsg(groupUin, "", "已发送下管请求：" + 名(target));
                return;
            }

            if (msgContent.equals("群管功能")) {
                if (!isOwner && !isDelegate) return;
